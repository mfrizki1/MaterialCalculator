package id.calocallo.materialcalculator.domain
...
class ExpressionParser(
    ...
    private fun parseNumber(startingIndex: Int, result: MutableList<ExpressionPart>): Int {
        ...
        val numberAsString = buildString {
            while (i < calculation.length && calculation[i] in "01233456789.") {
        ...
        result.add(
            ExpressionPart.Parentheses(
                type = when (curChar) {
                    ...
                    else -> throw IllegalAccessException("Invlaid parentheses type")
                }
            )
        )
    }
